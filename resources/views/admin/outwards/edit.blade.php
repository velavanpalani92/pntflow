...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.outward.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.outwards.update", [$outward->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('erp') ? 'is-invalid' : '' }}" name="erp_id" id="erp_id">
                    @foreach($erps as $id => $entry)
                        <option value="{{ $id }}" {{ (old('erp_id') ? old('erp_id') : $outward->erp->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('erp'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('serialno') ? 'is-invalid' : '' }}" name="serialno_id" id="serialno_id">
                    @foreach($serialnos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('serialno_id') ? old('serialno_id') : $outward->serialno->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id">
                    @foreach($categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $outward->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status_id" id="status_id">
                    @foreach($statuses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('status_id') ? old('status_id') : $outward->status->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
